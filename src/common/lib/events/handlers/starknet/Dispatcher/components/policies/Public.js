...
class Handler extends BaseHandler {
  static eventConfig = {
    keys: [
      ...
      '0x5075626c6963506f6c696379'
    ],
    name: 'ComponentUpdated_PublicPolicy'
  };
  ...
  async processEvent() {
    ...
    if (returnValues.public === false) {
      const { deletedCount } = await ComponentService.deleteOne({ component: 'PublicPolicy', data: returnValues });
    ...
    // Update packed lot data if policy is for lot use
  ...
  static transformEventData(event) {
    ...
    return {
      ...
      public: Boolean(Number(data.shift()))
    };
  }
}

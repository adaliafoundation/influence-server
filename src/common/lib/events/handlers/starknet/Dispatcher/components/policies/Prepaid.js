...
class Handler extends BaseHandler {
  static eventConfig = {
    keys: [
      ...
      '0x50726570616964506f6c696379'
    ],
    name: 'ComponentUpdated_PrepaidPolicy'
  };
  ...
  async processEvent() {
    ...
    if (returnValues.rate === 0 && returnValues.initialTerm === 0 && returnValues.noticePeriod === 0) {
      const { deletedCount } = await ComponentService.deleteOne({ component: 'PrepaidPolicy', data: returnValues });
      if (deletedCount > 0) deleted = true;
    } else {
      const { updated: _updated } = await ComponentService.updateOrCreateFromEvent({
        component: 'PrepaidPolicy',
        event: this.eventDoc,
  ...
  static transformEventData(event) {
    ...
    return {
      ...
      initialTerm: Number(data.shift()), // initial term in seconds
      noticePeriod: Number(data.shift()) // notice period in seconds
    };
  }
}

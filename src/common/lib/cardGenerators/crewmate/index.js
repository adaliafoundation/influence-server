...
const assetList = require('./assets');
...
// Generates a random crewmate, for fun and for checking for issues
const getRandomCrewmate = () => {
  const gender = Math.ceil(Math.random() * 2);
  const face = gender === 1 ? [0, 1, 3, 4, 5, 6, 7] : [0, 1, 2];
  return {
    name: '',
    collection: Math.ceil(Math.random() * 2),
    gender,
    body: (gender - 1) * 6 + Math.ceil(Math.random() * 6),
    crewClass: Math.ceil(Math.random() * 5),
    title: Math.ceil(Math.random() * 51),
    clothes: Math.ceil(Math.random() * 18),
    hair: (gender - 1) * 6 + Math.floor(Math.random() * 6),
    face: face[Math.floor(Math.random() * face.length)],
...
/**
 * Generates a crewmate card
 * @param crew The crewmate object
 * @param bustOnly Boolean indicating whether to generate with the card background or not
 * @param random Boolean which allows for generating a random crewmate
 */
const generateCard = async function ({ crewmateDoc = {}, fileType, options = { bustOnly: false, random: false } }) {
  const crewmate = (options.random) ? getRandomCrewmate() : crewmateDoc;
  const layers = [];
  ...
  if (options.random) {
    ({ gender, body, face, hair, hairColor, clothes, head, item, collection, crewClass, title, name } = crewmate);
  } else {
    if (crewmateDoc.Crewmate) {
  ...
  // Handle empty crewmate data (collection 0)
  if (collection === 0) layers.push({ input: `${itemsPath}/body/body${body}.png` });
  ...
  if (options.bustOnly) {
    image = await sharp(`${itemsPath}/transparent.png`).composite(layers).toBuffer();
    const texture = await sharp(image).composite([{ input: `${itemsPath}/texture.png`, blend: 'in' }]).toBuffer();
    image = await sharp(image).composite([{ input: texture, blend: 'soft-light' }]).toBuffer();
  } else {
    layers.push({ input: `${itemsPath}/texture.png`, blend: 'soft-light' });
    image = await sharp(`${itemsPath}/class/class${crewClass}.png`).composite(layers).toBuffer();
  }
  ...
  // Stop early if only returning the bust
  if (options.bustOnly) return outputCard(canvas, fileType);
  ...
  // Collection stamp
  if (collection) {
    const stampData = await datauri(`${itemsPath}/collection/collection${collection}.png`);
    canvas.image(stampData).size(225, 125).center(180, 1075);
  }

  // Custom name
  name = name || `Crewmate #${crewmate.id}`;
  canvas.text(name)
    .x(835).y(25)
    .font({ family: 'Jura', size: 50, weight: 100, anchor: 'end' })
    .style('fill', 'white');

  // Crew collection title
  if (collection) {
    canvas.text(Crewmate.getCollection(collection).name)
      .x(835).y(85)
  ...
  // Class title
  canvas.text(Crewmate.getClass(crewClass).name.toUpperCase())
    .x(835).y(1000)
    .font({ family: 'Jura', size: 40, weight: 100, anchor: 'end' })
    .style('fill', 'white');
  ...
  // Custom title
  if (title) {
    canvas.text(Crewmate.getTitle(title).name)
      .x(835).y(1050)
      .font({ family: 'Jura', size: 40, weight: 100, anchor: 'end' })
      .style('fill', '#bbbbbb');
  }

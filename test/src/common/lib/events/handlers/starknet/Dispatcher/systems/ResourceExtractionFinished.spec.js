...
const {
  ResourceExtractionFinished: Handler,
  ResourceExtractionStarted } = require('@common/lib/events/handlers/starknet/Dispatcher');

describe('ResourceExtractionFinished Handler', function () {
  let endEvent;
  let startEvent;
  ...
  beforeEach(function () {
    ...
    endEvent = mongoose.model('Starknet')({
      event: 'ResourceExtractionFinished',
      logIndex: 2,
  ...
  describe('processEvent', function () {
    ...
    it('should resolve the correct Activity Item', async function () {
      const { doc } = await ActivityService.findOrCreateOne({
        ...
        entities: [],
        event: startEvent,
        hashKeys: ResourceExtractionStarted.hashKeys,

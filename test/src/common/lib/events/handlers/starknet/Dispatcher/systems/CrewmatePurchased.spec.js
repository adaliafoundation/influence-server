...
const Handler = require('@common/lib/events/handlers/starknet/Dispatcher/systems/CrewmatePurchased');

describe('CrewmatePurchased Handler', function () {
  let event;
  const stubs = {
  ...
  describe('processEvent', function () {
    it('should create an Activity Item correctly', async function () {
      ...
      expect(handler.messages._messages).to.deep.equal([{ to: event.returnValues.caller }]);
    });
    ...
    it('should create an internal sale component', async function () {
      await mongoose.model('Constant').create({ name: 'ADALIAN_PURCHASE_PRICE', value: 500 });
      await (new Handler(event)).processEvent();

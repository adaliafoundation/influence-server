...
const Handler = require('@common/lib/events/handlers/ethereum/IStarknetCore/ConsumedMessageToL1');

describe('StarknetCore::ConsumedMessageToL1 event handler', function () {
  afterEach(function () {
  ...
  describe('processEvent', function () {
    it('should remove the corresponding crossing document', async function () {
      const eventDoc = mongoose.model('Ethereum')({
        ...
        returnValues: {
          ...
          fromAddress: BigInt(appConfig.get('Contracts.starknet.asteroid')).toString(),
    ...
    it('should decrement the ready count for the corresponding SwayCrossing document', async function () {
      const eventDoc = mongoose.model('Ethereum')({
        event: 'ConsumedMessageToL1',
      ...
      expect(doc.readyCount).to.equal(0);
